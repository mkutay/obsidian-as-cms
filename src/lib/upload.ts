import { type FrontMatterCache, TFile, type Vault } from "obsidian";
...
export async function upload(
  ...
  try {
    ...
    const coverImage = frontmatter?.cover
      ...
      : null;
    const coverSquareImage = frontmatter?.coverSquare
      ? (frontmatter.coverSquare as string)
      : null;
...
function findImage(relativePath: string, vault: Vault, currentFile: TFile) {
  ...
  if (!imageFile) {
    ...
    const matchedFile = attachmentFiles.find(
      (file) => file.path.endsWith(relativePath) || file.name === filename,
    );
      ...
      if (noteFolder) {
        ...
        for (const folderPath of possibleAttachmentFolders) {
          ...
          if (folder) {
            ...
            const potentialFile =
              vault.getAbstractFileByPath(potentialImagePath);
  ...
  for (const match of componentMatches) {
    ...
    if (stringUrl?.trim()) {
      ...
      // Handle simple variable names that might be filenames
        ...
        /^[a-zA-Z_$][a-zA-Z0-9_$]*\.(jpg|jpeg|png|gif|webp|svg|bmp|tiff)$/i.test(
          trimmed,
        )
      ) {
...
export function convertObsidianImageSyntax(content: string): string {
  ...
  return content.replace(/!\[\[(.*?)\]\]/g, (_match, imagePath) => {
